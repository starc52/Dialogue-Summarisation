...
class T5Stack(T5PreTrainedModel):
    ...
    def forward(
        ...
        if coref_information is not None:
            graph_attention_heads = []
            for i in range(len(coref_information)):
                graph_attention_heads.append(Adjacent_Coreference_Matrix(input_ids.size(1), coref_information[i][0], coref_information[i][1]))
            coref_information = torch.Tensor(np.array(graph_attention_heads)).to(self.device)
        ...
        for i, (layer_module, past_key_value) in enumerate(zip(self.block, past_key_values)):
            ...
            if not self.is_decoder and coref_information is not None:
                ...
                if i in replace_layers and add_coref_attn_layer is True:
